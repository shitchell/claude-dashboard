...
// TestMoveCursorPageUp verifies page up navigation.
func TestMoveCursorPageUp(t *testing.T) {
	m := NewModel(ModelConfig{})
	m.filteredSessions = make([]*session.Session, 20)
	for i := range m.filteredSessions {
		m.filteredSessions[i] = &session.Session{ID: string(rune('a' + i))}
	}
	m.cursorIndex = 15
	m.windowHeight = 10

	newModel, _ := m.moveCursorPageUp()
	updatedModel := newModel.(Model)

	// pageSize = 10 - 4 = 6, so cursor should go from 15 to 9
	expected := 15 - m.pageSize()
	if updatedModel.cursorIndex != expected {
		t.Errorf("expected cursorIndex=%d, got %d", expected, updatedModel.cursorIndex)
	}
}

// TestMoveCursorPageDown verifies page down navigation.
func TestMoveCursorPageDown(t *testing.T) {
	...
	newModel, _ := m.moveCursorPageDown()
	updatedModel := newModel.(Model)

	// pageSize = 10 - 4 = 6, so cursor should go from 5 to 11
	expected := 5 + m.pageSize()
	if updatedModel.cursorIndex != expected {
		t.Errorf("expected cursorIndex=%d, got %d", expected, updatedModel.cursorIndex)
	}
}
