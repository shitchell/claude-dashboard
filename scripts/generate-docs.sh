#!/usr/bin/env bash
#
# Generate documentation diagrams from Go AST
#
# Usage: generate-docs.sh [--check] [--force]
#
# Options:
...
function _generate-source-files() {
    ...
    if ! "${__goplantuml_bin}" -recursive "${PROJECT_ROOT}" > "${DIAGRAMS_DIR}/classes.puml.new"; then
        error "goplantuml failed"
        return ${E_ERROR}
    fi

    # Generate GraphViz source (use -format dot to skip SVG generation)
    info "Generating call graph source..."
    if ! (cd "${PROJECT_ROOT}" && "${__callvis_bin}" -format dot -file "${DIAGRAMS_DIR}/callgraph.new" . 2>&1) | grep -v "^$"; then
...
function _setup() {
    ...
    declare -gr PROJECT_ROOT="${SCRIPT_DIR}/.."
    declare -gr DIAGRAMS_DIR="${PROJECT_ROOT}/docs/diagrams/generated"

    # PlantUML config
    declare -gr PLANTUML_VERSION="1.2024.8"
    declare -gr PLANTUML_JAR="${PLANTUML_JAR:-/tmp/plantuml.jar}"
    declare -gr PLANTUML_URL="https://github.com/plantuml/plantuml/releases/download/v${PLANTUML_VERSION}/plantuml-${PLANTUML_VERSION}.jar"
